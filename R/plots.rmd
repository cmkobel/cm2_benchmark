...
df = df_raw %>% 
    ...
    comparison = case_when(
        pipeline %in% c("tormes", "asscom2") ~ "Tormes VS Assemblycomparator2",
        pipeline %in% c("bactopia", "asscom2bt") ~ "Bactopia VS Assemblycomparator2"
    ...
    pipeline = case_when(
            ...
            pipeline %in% c("asscom2", "asscom2bt") ~ "Assemblycomparator2",
...
df %>% 
    ...
    pivot_wider(names_from = genus, values_from = s, names_prefix = "seconds_") %>%  
    
    ungroup() %>% 
    mutate(
        factor_Methanobrevibacter = seconds_Methanobrevibacter/min(seconds_Methanobrevibacter),
        factor_Prevotella = seconds_Prevotella/min(seconds_Prevotella, na.rm = T),
    ) %>% 
